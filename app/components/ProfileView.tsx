...
interface ProfileViewProps {
  basePath: string; // e.g. "/dashboard" or "/student-dashboard"
}

export default function ProfileView({ basePath }: ProfileViewProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    country: "Bangladesh",
    education: "",
    role: "student", // Track role for display purposes
    designation: "",
    bio: "",
    image: "",
    expertise: "", // Store as string for input
  });
  ...
  const fetchProfile = async () => {
    try {
      ...
      setFormData((prev) => ({
        ...
        // Prefer the unified User fields, fallback to student if empty
        mobileNo: data.user?.mobileNo || data.student?.mobileNo || "",
        presentAddress: data.user?.presentAddress || data.student?.presentAddress || "",
        fatherName: data.user?.fatherName || data.student?.fatherName || "",
        motherName: data.user?.motherName || data.student?.motherName || "",
        guardianMobileNo: data.user?.guardianMobileNo || data.student?.guardianMobileNo || "",
        dateOfBirth: data.user?.dateOfBirth || data.student?.dateOfBirth || "",
        nidNo: data.user?.nidNo || data.student?.nidNo || "",
        gender: data.user?.gender || data.student?.gender || "Male",
        maritalStatus: data.user?.maritalStatus || data.student?.maritalStatus || "Single",
        residentialStatus: data.user?.residentialStatus || data.student?.residentialStatus || "Resident",
        country: data.user?.country || data.student?.country || "Bangladesh",
        education: data.user?.education || data.student?.education || "",
        designation: data.user?.designation || "",
        bio: data.user?.bio || "",
        image: data.user?.image || "",
        expertise: Array.isArray(data.user?.expertise) ? data.user.expertise.join(", ") : (data.user?.expertise || ""),
      }));
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="w-full pb-12">
      ...
      {/* Page Header Area */}
      <div className="bg-white border-b border-gray-100 py-8 mb-8 shadow-sm rounded-xl">
        <div className="px-6 md:px-8">
          <div className="flex items-center gap-2 text-sm text-gray-400 font-medium mb-3">
            <Link href={basePath} className="hover:text-[#6C5DD3] transition-colors">Dashboard</Link>
            <span>/</span>
            ...
            {formData.role === 'teacher' && (
              <>
                ...
                <div className="mb-8">
                  ...
                  <p className="text-sm text-gray-500">How you appear to students and the public.</p>
                </div>
