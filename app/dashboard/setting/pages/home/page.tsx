...
export default function HomePageSettings() {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: 'hero' | 'testimonial' | 'avatar' = 'hero') => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // We return the URL so callers (e.g. Testimonials) can handle it specifically
    ...
    const handleSaveTestimonial = async () => {
        ...
        if (testimonialView === 'edit' && currentTestimonial._id) {
            updatedTestimonials = baseTestimonials.map(t => t._id === currentTestimonial._id ? currentTestimonial : t);
        } else {
            updatedTestimonials = [...baseTestimonials, currentTestimonial];
        }
    ...
    const handleDeleteTestimonial = async (index: number) => {
        ...
        await saveSiteSettings({ testimonials: updatedTestimonials });
    };
                    ...
                    <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-gray-500 font-medium">Add student feedback and video reviews</p>
                            </div>
                        ...
                        {testimonialView === 'list' ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {testimonials.length > 0 ? (
                                    testimonials.map((item, index) => (
                                        <div key={index} className="group border-2 border-gray-50 rounded-[2rem] overflow-hidden bg-white hover:border-[#6C5DD3]/20 hover:shadow-2xl hover:shadow-[#6C5DD3]/10 transition-all duration-500">
                                            <div className="h-48 bg-gray-100 relative overflow-hidden">
                                                <img src={item.image || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=400&auto=format&fit=crop"} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={item.studentName} />
                                                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-end p-4">
                                                    <div>
                                                        <p className="text-white font-black text-sm">{item.studentName}</p>
                                                        <p className="text-white/70 text-[10px] font-bold uppercase tracking-wider">{item.courseName}</p>
                                                    </div>
                                                </div>
                                                <div className="absolute top-4 right-4 flex gap-2 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all">
                                                    <button onClick={() => handleEditTestimonial(item)} className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-[#6C5DD3] hover:scale-110 transition-all shadow-lg"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg></button>
                                                    <button onClick={() => handleDeleteTestimonial(index)} className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-red-500 hover:scale-110 transition-all shadow-lg"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="3 6 5 6 21 6" /><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" /></svg></button>
                                                </div>
                                            </div>
                                            <div className="p-5">
                                                <p className="text-xs text-gray-500 line-clamp-2 font-medium italic">"{item.textFeedback}"</p>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    <div className="col-span-full py-20 text-center bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200">
                                        <p className="text-gray-400 font-black uppercase tracking-widest">No testimonials added yet</p>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <form onSubmit={(e) => { e.preventDefault(); handleSaveTestimonial(); }} className="space-y-8 animate-fade-in">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    ...
                                    <div className="space-y-6">
                                        <div className="space-y-2">
                                            <label className="text-sm font-bold text-gray-700 ml-1">Video ID / URL</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="space-y-4">
                                            <label className="text-sm font-bold text-gray-700 ml-1">Student Image</label>
                                            <div className="flex flex-wrap items-center gap-6 p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
                                                <div className="w-24 h-24 rounded-2xl overflow-hidden bg-gray-200 border border-gray-200 shrink-0">
